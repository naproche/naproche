...
headed = qu_statem <|> if_then_statem <|> wrong_statem
  where
    ...
    wrong_statem   = mapM_ wd_token ["it", "is", "wrong", "that"] >> fmap Not statement
...
chained = label "chained statement" $ andOr <|> neitherNor >>= chainEnd
  where
    ...
    andChain f =
      fmap (foldl And f) $ wd_token "and" >> atomic `sepBy` wd_token "and"
    ...
    orChain  f = fmap (foldl Or  f) $ wd_token "or"  >> atomic `sepBy` wd_token "or"
...
chainEnd f = optLL1 f $ and_st <|> or_st <|> iff_st <|> where_st
  where
    and_st   = fmap (And f) $ wd_token "and" >> headed
    or_st    = fmap (Or  f) $ wd_token "or"  >> headed
...
has_predicat = label "has predicat" $
  ...
  where
    ...
    common     = wd_token "common" >> (fmap multExi $ (fmap digadd possess) `sepBy` comma)
...
basentn = fmap digadd $ cm <|> sym_eqnt <|> (set </> prim_ntn term)
  where
...
anotion =
  ...
  where
    ...
    rat = fmap (Tag Dig) stattr ---- why here Tag Dig??
...
terms = label "terms" $
  ...
  where
    m_term     = qu_notion -|- fmap s2m definite_term
    s2m (q, t) = (q, [t])
...
s_form  = s_iff
  where
    ...
    s_not = fmap Not $ wd_token "not" >> s_una
    s_dot = wd_token ":" >> s_form
    s_atm = s_atom

    qua_f qu op (_, f, v) = flip (foldr qu) v . op f

    bin_f op p f = optLL1 f $ fmap (op f) p
...
s_atom = s_relation -|- expar statement
  where
    ...
    s_chain = fmap (foldl1 And . concat) s_hd

    s_hd  = l_hd -|- (s_ts >>= s_tl)
    l_hd  = do
      pr <- prim_lpr s_term; rs <- s_ts
      fmap (map pr rs :) $ opt [] $ s_tl rs

    s_tl ls = i_tl ls -|- r_tl ls
    i_tl ls = do
      pr <- prim_ipr s_term; rs <- s_ts
      fmap (liftM2 pr ls rs :) $ opt [] $ s_tl rs
...
elementCnd = setTerm </> fmap fst symb_set_notation
  where
...
chooseInTerm = do
  ...
  where
    ...
    term    = fmap ((.) (Tag Evaluation) . flip zEqu) s_term
    defTerm = do
...
conj_chain = fmap (foldl1 And) . flip sepBy (wd_token "and")

qu_chain  = fmap (foldl fld id) $ wd_token "for" >> qu_notion `sepByLL1` comma -- we can use LL1 here, since there must always follow
