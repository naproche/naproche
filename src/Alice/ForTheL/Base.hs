...
prim_cpr = getExpr cpr_expr . prim_csm
prim_rpr = getExpr rpr_expr . prim_rsm
prim_lpr = getExpr lpr_expr . prim_lsm
prim_ipr = getExpr ipr_expr . prim_ism
...
cm_patt p s (Nm : Wd l : Vr : ls) = do
  guard $ elem "of" l; vs <- namlist; wd_tokenOf l
  (r, as) <- s
...
na_patt p (Wd l : ls) = guard (notElem "and" l) >> wd_tokenOf l >> wd_patt p ls
