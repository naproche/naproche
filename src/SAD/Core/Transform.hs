...
instance Semigroup Context where
  (<>) (Context t1 r1 n1 c1) (Context t2 r2 n2 c2) = Context (t1 <> t2) (r1 <> r2) (n1 <> n2) (c1 <> c2)
...
parseType (Context idents _ _ _) = \case 
...
resolve :: Context -> TermName -> Type -> (TermName, Type)
resolve (Context idents res _ coe) name typ =
...
gatherTypes c = snd . go Prop mempty 0
  where
    ...
    go context typings d = \case
      ...
      Var v -> case context of
        ...
        InType Object -> case Map.lookup v typings of
          ...
          Just t -> (InType $ runIdentity t, Var v)
...
subClaims :: Context -> [Either Proof Block] -> [Proof]
subClaims ctx xs = flip map xs $ \case
  Left p -> p
  Right b -> subClaim ctx b

chooses :: Context -> [Either Proof Block] -> [Either Proof Block]
chooses ctx = go
  where
    go [] = []
...
convertProof ctx = subClaims ctx . chooses ctx
  . cases ctx
    ...
    (F.Var (VarHole _) _ _) ->
      ...
      in case Block.needsProof bl of
        True -> if not (null defs)
          ...
          else [Claim (noTags mainT) l (convertProof ctx (body main))] -- TODO
        False -> case defs of
...
convert = go mempty . concatMap (\case ProofTextBlock bl -> [bl]; _ -> [])
  where
    ...
    addStmt (Context idents res nn coe) (Located _ _ s) = case s of
      IntroSort n ->
        let n' = newName n res_n 
            res_n = Map.lookup n res
            res_n' = fromMaybe mempty res_n
        in Context (Map.insert n' Sort idents) (Map.insert n (Set.insert n' res_n') res) nn coe
      Predicate n is o ->
        let n' = newName n res_n 
            res_n = Map.lookup n res
            res_n' = fromMaybe mempty res_n
        in Context (Map.insert n' (Pred is o) idents) (Map.insert n (Set.insert n' res_n') res) nn coe
      Coercion n from to -> Context idents res nn (add (from, to) n coe)
      _ -> Context idents res nn coe