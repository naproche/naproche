-- |
-- Module      : SAD.ForTheL.STEX.Statement
...
classEquality = label "a class equality" $ twoClassTerms </> oneClassTerm
  where
    twoClassTerms = do
      cnd1 <- fmap stripClass symbClassNotation; texCommand "Eq"
...
classNotion = do
  v <- after var (texCommand "DefEq"); (_, f, _) <- collection
...
symbClassNotation = stexEnumerationClass </> stexComprehensionClass </> stexSeparationClass
  where
...
mapNotion = sVar <**> (wordMap <|> (texCommand "DefEq" >> lambda))
  where
  wordMap = do
...
chooseInTerm = do
  ...
  where
    ...
    def = do
      markupToken Reports.lowlevelHeader "define"; x <- var; xDecl <- makeDecl x; texCommand "DefEq"
      ap <- ld_class <|> lambda
