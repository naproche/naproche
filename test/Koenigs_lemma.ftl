...
Definition.
Assume M is a subset of the domain of f. f^[M] = { f[x] | x is an element of M }. Let the image of f stand for f^[Dom(f)].
...
Axiom.
Assume card(M) =< card(N). There exists a function f such that N is the domain of f and M is the image of f.
...
Axiom Prod_Def.
let kappa be a sequence of cardinals on D. ProdSet(kappa,D) = { function f | Dom(f) = D /\ (f[i] is an element of kappa[i] for every element i of D) }.
...
Proof.
Assume the contrary. Then Prod (lambda,D) =< Sum(kappa,D). Take a function G such that SumSet(kappa,D) is the domain of G and ProdSet(lambda,D) is the image of G.
Define Diag[i] = { G[(n,i)][i] | n is an element of kappa[i] } for i in D.
For every element i of D card(Diag[i]) < lambda[i].[ontored off]
