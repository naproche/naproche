...
by K. Buzzard, J. Commelin, and P. Massot \cite{Buzzard} was a milestone in bringing 
...
\cite{Naproche}, in contrast, accepts and checks 
...
We define \textit{perfectoid rings} in the proof assistant \Naproche{} \cite{Naproche} 
with the intention to closely approximate the mathematical 
\textit{language} 
of Definition 3.1 of
P. Scholze's \cite{Scholze}:
\begin{quote}
...
as its
input language (see also \cite{Paskevich})
...
to an external automated theorem prover (ATP). We mostly use the E first-order prover \cite{Schulz} but also Vampire \cite{Vampire}.
...
We also use material from \cite{Wedhorn}.
...
Let $R$ denote a topological ring.
\end{forthel}
The following is the original definition of f-adic ring by R. Huber \cite{Huber}:
\begin{forthel}
...
\bibitem{perfectoid} Peter Scholze: Perfectoid Spaces.
