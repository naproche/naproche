...
\abstract{The formalization of P. Scholze's \emph{perfectoid spaces} 
\cite{Scholze2012}
...
After defining a group as a \emph{set} with further structure, we can now
define a ring as a \emph{group} together with multiplication and a $1$.
...
Let us now hide the parameter $G$ as we did previously.
\catcode`^\active
\def^#1{\ifthenelse{\equal{#1}{G}} {\unskip} {\sp{#1}}}
(Note that $R$ is no longer ``hidden'').
...
\end{forthel}
Let us hide \verb|^{R}| and \verb|,R}| as we have done before.
\catcode`^\active
\def^#1{\ifthenelse{\equal{#1}{R}} {\unskip} 
       {{\IfEndWith{#1}{,R}{\sp{\StrBefore{#1}{,R}}}{\sp{#1}}}}}
