...
\begin{proof}[forthel]
  \begin{case}{$n \NATmul k \NATless m \NATmul k$.}
    Define $\Phi = \{ n' \SETin \Nat \mid\text{ if }n' \NATmul k \NATless m \NATmul k\text{ then }n' \NATless m \}$.
...
\end{proposition}
\begin{proof}[forthel]
  Define $\Phi = \{ n' \SETin \Nat \mid\text{ if }n', m \NATgtr k\text{ then }n' \NATmul m \NATgtr k \}$.
