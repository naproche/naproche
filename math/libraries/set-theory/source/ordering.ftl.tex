...
\begin{proof}[forthel]
  ...
  Define $A = \{ \alpha' \SETin \Ord \mid\text{ there exists an ordinal }\beta'\text{ such that neither }\alpha' \ORDless \beta'\text{ nor }\alpha' \eq \beta'\text{ nor }\alpha' \ORDgtr \beta' \}$.
  
  $A$ is nonempty.
  Hence we can take a $\alpha' \SETin A$ such that for no $\gamma \SETin A$ we have $\gamma \ORDless \alpha'$.
  
  Define $B = \{ \beta' \SETin \Ord \mid\text{ neither }\alpha' \ORDless \beta'\text{ nor }\alpha' \eq \beta'\text{ nor }\alpha' \ORDgtr \beta' \}$.
