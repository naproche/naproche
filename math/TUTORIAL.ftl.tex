...
\usepackage{examples/lang/naproche}
\input{examples/lang/preamble}

\newenvironment{exercise}{\noindent\textbf{Exercise. }}{\par}
\newenvironment{greybox}{\forthel}{\endforthel}
...
in that format, lines starting with a \texttt{\#} % We must escape '#'
% (and hence use \texttt instead of \verb) since otherwise FTLex would
% interpret '#' as a parameter character with a missing number.
...
with \texttt{\%}. They can also be used to force rechecking of 
the text: uncomment
...
\end{lemma}
\begin{proof}[method=induction on $m$] \end{proof}
\end{forthel}
...
\begin{axiom}[id=Factorproperty] Let $F$ be a sequence of length $n$
