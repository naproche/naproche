...
Axiom.
Let x be a square.
Then x = (m,n) for some integer mod eight m and some integer mod eight n.
...
Axiom.
The checkerboard is a set.

Let \Check stand for the checkerboard.

Axiom.
Every subset of \Check is Dedekind finite.

Definition.
\Corners = {(0,0), (7,7)}.

Definition.
\Mutil = \Check \setminus \Corners.

Let the mutilated checkerboard stand for \Mutil.

Signature.
Let m be an integer.
A neighbour of m is an integer.
...
Definition.
x is adjacent to y iff there exists integers b, c, d, e such that x = (b,c) and
y = (d, e) and ((b = d and c ~ e) or (b ~ d and c = e)).
...
A domino tiling of A is a domino tiling T such that for every square x
x is an element of A iff x is an element of some element of T.

Signature.
x is black is a relation.
...
Axiom.
If x is adjacent to y then x is black iff y is white.

Definition.
\Blck is the class of black elements of the checkerboard.

Definition.
\Wht is the class of white elements of the checkerboard.

Lemma.
\Blck is a set.
Indeed \Blck is a subclass of the checkerboard.

Lemma.
\Wht is a set.
Indeed \Wht is a subclass of the checkerboard.
...
Axiom.
Swap(0,n) = (1,n) and Swap(1,n) = (0,n).

Axiom.
Swap(2,n) = (3,n) and Swap(3,n) = (2,n).

Axiom.
Swap(4,n) = (5,n) and Swap(5,n) = (4,n).

Axiom.
Swap(6,n) = (7,n) and Swap(7,n) = (6,n).
...
Sw(T,A,x) is a square y such that there is an element D of T such that D = {x,y}.
...
Let A be a subset of the checkerboard.
Let T be a domino tiling of A.
Then A \cap \Blck is equinumerous with A \cap \Wht.
Proof.
  Define F(x) = Sw(T,A,x) for x in A \cap \Blck.
  Define G(x) = Sw(T,A,x) for x in A \cap \Wht.
  F : A \cap \Blck \rightarrow A \cap \Wht.
  G : A \cap \Wht \rightarrow A \cap \Blck.
  For all elements x of A \cap \Blck we have G(F(x)) = x.
  For all elements x of A \cap \Wht we have F(G(x)) = x.
  Thus F : A \cap \Blck \leftrightarrow A \cap \Wht.
End.
...
Lemma.
\Mutil \cap \Blck is a proper subset of \Blck.
Proof.
  (0,0) is an element of \Blck.
  (0,0) is not an element of \Mutil.
  Thus (0,0) is not an element of \Mutil \cap \Blck.
End.
...
Proof by contradiction.
  ...
  \Mutil \cap \Blck is equinumerous with \Mutil \cap \Wht.
  \Mutil \cap \Blck is equinumerous with \Wht.
  \Mutil \cap \Blck is equinumerous with \Blck.
  Contradiction.
End.
