...
Definition.
The powerset of S is the collection of subsets of S.
...
Definition.
The intersection of S and T is {x in S | x \in T}.

Let S \cap T stand for the intersection of S and T.

Definition.
The union of S and T is {x | x \in S \/ x \in T}.

Let S \cup T stand for the union of S and T.

Definition.
The set difference of S and T is {x in S | x \notin T}.
...
Definition.
A disjoint family is a family F such that X is disjoint from Y for all nonequal
elements X, Y of F.
...
Definition.
S \times T = {(x,y) | x is an element of S and y is an element of T}.
...
Axiom.
Assume that Dom(f) is a set and f(x) is an object for any element x of Dom(f).
...
Let the range of f stand for the image of f.
Let Ran(f) stand for the range of f.

Definition.
A function on S is a function f such that Dom(f) = S and Ran(f) \subseteq S.
...
Definition.
f : S \leftrightarrow T iff f : S \rightarrow T and there exists a map g such
that g : T \rightarrow S and g retracts f and g sections f.
...
Definition.
S is equinumerous with T iff there exists a map F such that
F : S \leftrightarrow T.

Lemma.
Assume that S is equinumerous with T.
Then T is equinumerous with S.

Lemma.
Assume S is equinumerous with T.
Assume T is equinumerous with U.
Then S is equinumerous with U.
Proof.
  Take a map F such that F : S \leftrightarrow T.
  Take a map G such that G : T \rightarrow S and
  (for all elements x of S we have G(F(x)) = x) and
  (for all elements y of T we have F(G(y))=y).
  Take a map H such that H : T \leftrightarrow U.
  Take a map I such that I : U \rightarrow T and
  (for all elements x of T we have I(H(x)) = x) and
  (for all elements y of U we have H(I(y))=y).
  For every element x of S H(F(x)) is an object.
  Define J(x) = H(F(x)) for x in S.
  For every element y of U G(I(y)) is an object.
  Define K(y) = G(I(y)) for y in U.
  K: U \rightarrow S.
  K retracts J and K sections J.
  J: S \leftrightarrow U.
End.
