...
that aims to approximate the 
language, structure and appearance of common mathematical texts;
FortheL texts are checked for syntactic, ontological 
...
running \Naproche{} server process.

This document is an introduction to some principles and to the 
use of the \Naproche{} prover. We introduce ForTheL commands and statements by
examples taken from a proof of the infinitude of primes.
...
k + l is a natural number./ generates an
...
Let us consider a proof by induction:
...
(1) $\val{p}{i}$ is a nonzero natural number for every $i \in Dom(p)$.
Consider $n=\Prod{p}{1}{r}+1$.
$\Prod{p}{1}{r}$ is nonzero. 
Hence $n$ is nontrivial. Take a prime divisor $q$ of $n$.

Let us show that $q \neq \val{p}{i}$ for all natural numbers $i$ such that 
$1 \leq i \leq r$.

Proof by contradiction.
Assume the contrary.
Take a natural number $i$ such that $1 \leq i \leq r$ and $q=\val{p}{i}$.
