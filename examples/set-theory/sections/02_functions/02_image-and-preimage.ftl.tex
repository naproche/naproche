...
\begin{document}
  ...
  \begin{forthel}
    ...
    \end{lemma}
    \begin{proof}
      Take $y = \range(f \restriction_{(\dom(f) \cap z)})$.
      Then \[ y = \class{(f \restriction_{(\dom(f) \cap z)})(u) | u \in \dom(f) \cap z}. \]
    ...
    \begin{corollary}\label{SetTheory_02_02_216993}
      Let $f$ be a function from $x$ to $y$ and $a \subseteq x$.
      Then $f[a] = \range(f \restriction_{a})$.
    \end{corollary}
    \begin{proof}
      We have $f[a] = \class{f(u) | u \in a}$.
      Hence $f[a] = \range(f \restriction_{a})$.
    \end{proof}
    ...
    \begin{proposition}\label{SetTheory_02_01_257685}
      ...
      Then $f \restriction_{a}$ is a bijection between $a$ and $f[a]$.
    \end{proposition}
    \begin{proof}
      (1) $f \restriction_{a}$ is a function of $a$.

      (2) $f \restriction_{a}$ is one to one.

      (3) $\range(f \restriction_{a}) = f[a]$.
      Proof.
        Let us show that $\range(f \restriction_{a}) \subseteq f[a]$.
          Let $v \in \range(f \restriction_{a})$.
          Take $u \in a$ such that $v = (f \restriction_{a})(u)$.
          Then $v = f(u)$.
          Hence $v \in f[a]$.
        End.

        Let us show that $f[a] \subseteq \range(f \restriction_{a})$.
          Let $v \in f[a]$.
          Take $u \in a$ such that $v = f(u)$.
          Then $v = (f \restriction_{a})(u)$.
          Hence $v \in \range(f \restriction_{a})$.
        End.
      Qed.

      Thus $f \restriction_{a}$ is an one to one function from $a$ onto $f[a]$.
      Therefore $f \restriction_{a}$ is a bijection between $a$ and $f[a]$.
    \end{proof}
  \end{forthel}
